...
//                        ->where(['tbl_storage_location.item_no'=>$row->item_no,'comp_id'=>$user_comp_id])
                        ->where(['tbl_storage_location.item_no'=>$row->item_no])
                       // ->havingRaw('stock_qty <= '.$row->threshold_qty)
                        ->first(); 
                $order_qty=DB::table('tbl_order')
                        ->select(DB::raw('SUM(tbl_order.given_quntity) AS used_qty'),DB::raw('SUM(tbl_order.return_qty) AS return_qty'))
                        ->where(['tbl_order.purchase_status'=>1,'item_no'=>$row->item_no,'tbl_order.comp_id'=>$user_comp_id])
                        ->first();
                $order_qty1=DB::table('tbl_order')
                        ->select(DB::raw('SUM(tbl_order.given_quntity) AS used_qty'),DB::raw('SUM(tbl_order.return_qty) AS return_qty'))
                        ->where(['tbl_order.purchase_status'=>1,'item_no'=>$row->item_no,])
                        ->first();
        ?>
        <tr>
            <td>{{$x++}}</td>
            <!--<td>{{$row->item_no}}</td>-->
            <td>{{$row->item_desc}}</td>
            <td>{{($stock_qty->stock_qty + $order_qty1->used_qty - $order_qty1->return_qty)}}</td>
            <td>{{$stock_qty->stock_qty}}</td>
            <td>{{($order_qty->used_qty - $order_qty1->return_qty)}}</td>
